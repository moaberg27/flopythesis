...
def _save_head_plot(dfn, regbox, out_path_stem):
    ...
    try:
        ...
        dfn.plot_fractures_head(p, 40, 10, opacity=1, contour=True) # increase 10 for higher resolution
        regbox.plot(p)
...
def run_dfn_simulation(dfn_org, snapshot, phi_deg, theta_deg, face_index,
    ...
    try:
        ...
        dfn.solve(unconsolidate=True) ## TRUE FOR PT
...
if __name__ == "__main__":
    ...
    path = os.path.join(SCRIPT_DIR, "40000.csv")
    if not os.path.isfile(path):
        raise FileNotFoundError(f"Input fracture file not found: {path}")
    logger.info("DFN importing from file")
    ...
    paper_pts = sphere_pts @ eigvecs_spd * np.sqrt(eigvals_plot)
