...
ellipse_x = eigvals[0] * np.cos(theta_fit) * np.cos(np.radians(principal_angle)) - eigvals[1] * np.sin(theta_fit) * np.sin(np.radians(principal_angle))
ellipse_y = eigvals[0] * np.cos(theta_fit) * np.sin(np.radians(principal_angle)) + eigvals[1] * np.sin(theta_fit) * np.cos(np.radians(principal_angle))
plt.figure(figsize=(6, 6))
plt.plot(np.real(points_x_pos), np.imag(points_x_pos), 'o', label='Effective Conductivity (x)')
plt.plot(np.real(points_y_pos), np.imag(points_y_pos), 's', label='Effective Conductivity (y)')
plt.plot(np.real(points_x_neg), np.imag(points_x_neg), 'o', label='Effective Conductivity (-x)')
plt.plot(np.real(points_y_neg), np.imag(points_y_neg), 's', label=' Effective Conductivity (-y)')
plt.plot(ellipse_x, ellipse_y, 'k--', label='Fitted Ellipse')
plt.xlabel('Conductivity in x-direction')
plt.ylabel('Conductivity in y-direction')
plt.title('Effective Conductivity with Fitted Ellipse')
plt.legend()
plt.axis('equal')
plt.grid()
# plot principal axes
origin = [0], [0]
plt.quiver(*origin, eigvecs[0, 0], eigvecs[1, 0], color='r', scale=5, label='Principal Axis 1')
plt.quiver(*origin, eigvecs[0, 1], eigvecs[1, 1], color='b', scale=5, label='Principal Axis 2')

# Annotate principal values on the plot
plt.text(0.05, 0.95, f"Principal values:\n$k_1$ = {eigvals[0]:.3f}\n$k_2$ = {eigvals[1]:.3f}",
