...
dfn_results = {} # defining a dictionary to store results for each rotation angle (0° to 75°, step 15°)
...
def mock_dfn(angle_deg, k_major=5.0, k_minor=1.0, principal_dir=30.0, noise=0.20):

    def _k_directional(global_angle_deg): # Computes the effective K for a given global angle based on an underlying anisotropic tensor with principal values k_major and k_minor, oriented at principal_dir, and adds some noise to simulate variability in the DFN results.
    ...
    # ----------------------------------------------------------------------------
    # The 4 BC experiments probe these global directions based on the rotation angle:
