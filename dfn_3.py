...
def run_dfn_simulation(dfn_org, snapshot, phi_deg, theta_deg, face_index):
    ...
    try:
        ...
        dH = abs(HEAD_HIGH - HEAD_LOW)
        L  = BOX_SIZE
        A  = BOX_SIZE * BOX_SIZE
        return abs(Q) * L / (A * dH)  # conductivity magnitude from Darcy's law
...
if __name__ == "__main__":
    ...
    # ---------------------------------------------------------------
    # Drop failed runs (NaN) and non-physical k values (zero/negative or non-finite).
    # With the hydraulic-gradient magnitude used above, invalid points here are
    # typically disconnected or numerically failed DFN solves, which would poison
